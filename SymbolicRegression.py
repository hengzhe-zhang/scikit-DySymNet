...
from .scripts import pretty_print, functions
...
from .scripts.utils import nrmse, R_Square
...
class DySymNetRegressor:
    def __init__(self, config, func=None, func_name=None, data_path=None):
        ...
        if self.func is not None:
            ...
            if config.NOISE > 0:
                ...
                self.x_test, self.y_test = generate_data(
                    func,
                    self.config.N_TRAIN,
                    range_min=self.config.DOMAIN_TEST[0],
        ...
        # }
        # with open(
        #     os.path.join(self.results_dir, "args_{}.txt".format(self.func_name)), "a"
        # ) as f:
    ...
    def agent_init(self):
        self.dtype = (
            self.X.dtype
        )  # obtain the data type, which determines the parameter type of the model
        ...
        self.agent = Agent(
            ...
            num_funcs_avail=len(self.funcs_avail),
            n_layers=self.n_layers,
            num_funcs_layer=self.num_func_layer,
            device=self.device,
            dtype=self.dtype,
        )
    ...
    def solve_environment(self):
        ...
        best_expression, best_performance, best_relative_error = (
            None,
            float("-inf"),
            float("inf"),
        )
        ...
        # f1 = open(
        #     os.path.join(self.results_dir, "eq_{}_all.txt".format(self.func_name)), "a"
        # )
        # f1.write("\n{}\t\t{}\n".format(self.now_time, self.func_name))
        # f1.write(
        #     "{}\t\tReward\t\tR2\t\tExpression\t\tnum_layers\t\tnum_funcs_layer\t\tfuncs_per_layer\n".format(
        #         self.reward_type
        #     )
        # )
        #
        ...
        # f2 = open(
        #     os.path.join(self.results_dir, "eq_{}_summary.txt".format(self.func_name)),
        #     "a",
        # )
        # f2.write("\n{}\t\t{}\n".format(self.now_time, self.func_name))
        ...
        if self.optimizer == "Adam":
            optimizer = torch.optim.Adam(
                self.agent.parameters(), lr=self.config.learning_rate1
            )
        else:
        ...
        for i in range(self.num_epochs):
            ...
            while j < self.batch_size:
                ...
                ) = self.play_episodes()  # play an episode
            ...
            #     "{}\t\t{:.8f}\t\t{:.8f}\t\t{:.8f}\t\t{}\n".format(
            #         i,
            #         relative_error_list[np.argmax(r2.cpu())],
            #         max(rewards).item(),
            #         max(r2).item(),
            #         best_r2_expression,
            #     )
            ...
            indices_to_keep = torch.tensor(
                [j for j in range(len(rewards)) if rewards[j] > threshold],
                device=self.device,
            )
            ...
            risk_seeking_grad = torch.sum(
                (sub_rewards - threshold) * sub_log_probs, dim=0
            )
            entropy_grad = torch.sum(sub_entropies, dim=0)

            # Mean reduction and clip to limit exploding gradients
            risk_seeking_grad = torch.clip(
                risk_seeking_grad / (self.config.risk_factor * len(sub_rewards)),
                min=-1e6,
                max=1e6,
            )
            entropy_grad = self.config.entropy_weight * torch.clip(
        ...
        #     os.path.join(
        #         self.results_dir,
        #         "reward_{}_{}.txt".format(self.func_name, self.now_time),
        #     ),
        #     "w",
        # )
        ...
        if self.config.plot_reward:
            ...
            plt.plot(
                [i + 1 for i in range(len(epoch_mean_rewards))], epoch_mean_rewards
            )  # mean reward of full epoch
            plt.xlabel("Epoch")
            plt.ylabel("Reward")
            ...
            plt.savefig(
                os.path.join(
                    self.results_dir,
                    "reward_{}_{}.png".format(self.func_name, self.now_time),
                )
            )

        if early_stopping:
            self.best_expression = eq
            return eq, R2, error, relative_error
        else:
            self.best_expression = best_expression
            return (
                best_expression,
                best_performance.item(),
                1 / max(rewards).item() - 1,
                best_relative_error,
            )
    ...
    def bfgs(self, eq, X, y, n_restarts):
        ...
        c = symbols(
            "c0:10000"
        )  # Suppose we have at most n constants, c0, c1, ..., cn-1
        consts = list(
            expr.atoms(Float)
        )  # Only floating-point coefficients are counted, not power exponents
        consts_dict = {
            c[i]: const for i, const in enumerate(consts)
        }  # map between c_i and unoptimized constants
        ...
        for i in range(n_restarts):
            ...
            # bfgs optimization
            fun_timed = TimedFun(
                fun=sp.lambdify(s, loss_func, modules=["numpy"]), stop_after=int(1e10)
            )
            if len(x0):
                minimize(
            ...
            if isinstance(y_pred, float):
                ...
                y_pred = torch.where(
                    torch.isinf(y_pred), 10000, y_pred
                )  # check if there is inf
                y_pred = torch.where(
                    y_pred.clone().detach() > 10000, 10000, y_pred
                )  # check if there is large number
                R2 = max(0.0, R_Square(y.squeeze(), y_pred))
                loss_eq = torch.mean(torch.square(y.squeeze() - y_pred)).item()
                relative_error = torch.mean(
                    torch.abs((y.squeeze() - y_pred) / y.squeeze())
                ).item()
            R2_list.append(R2)
    ...
    def play_episodes(self):
        ...
        init_state = torch.rand(
            (1, self.input_size), device=self.device, dtype=self.dtype
        )  # initial the input state
        ...
        if self.auto:
            (
                ...
                init_state
            )  # output the symbolic network structure parameters
        ...
        print(
            "Operators of each layer obtained by sampling: ", self.funcs_per_layer_name
        )
    ...
    def train(self, trials=1):
        ...
        test_data, test_target = self.x_test.to(self.device), self.y_test.to(
            self.device
        )
        ...
        n_double_per_layer = [
            functions.count_double(f) for f in self.funcs_per_layer.values()
        ]

        if self.auto:
            init_stddev = (
                [self.init_sd_first]
                + [self.init_sd_middle] * (self.n_layers - 2)
                + [self.init_sd_last]
            )
                    ...
                    initial_weights=[
                        ...
                        torch.fmod(
                            torch.normal(
                                ...
                                size=(
                                    self.x_dim,
                                    width_per_layer[0] + n_double_per_layer[0],
                                ),
                            ),
                            2,
                        ),
                        ...
                        torch.fmod(
                            torch.normal(
                                ...
                                size=(
                                    width_per_layer[0],
                                    width_per_layer[1] + n_double_per_layer[1],
                                ),
                            ),
                            2,
                        ),
                        torch.fmod(
                            torch.normal(
                                ...
                                size=(
                                    width_per_layer[1],
                                    width_per_layer[2] + n_double_per_layer[2],
                                ),
                            ),
                            2,
                        ),
                        torch.fmod(
                            torch.normal(
                                0, self.init_sd_last, size=(width_per_layer[-1], 1)
                            ),
                            2,
                        ),
                    ],
                ...
                for epoch in range(self.config.n_epochs1 + 2000):
                    ...
                    if epoch % self.config.summary_step == 0:
                        ...
                        with torch.no_grad():  # test error
                            test_outputs = net(
                                test_data
                            )  # [num_points, 1] as same as test_target
                            ...
                            test_outputs = torch.where(
                                torch.isnan(test_outputs),
                                torch.full_like(test_outputs, 100),
                                test_outputs,
                            )
                        ...
                        if self.config.verbose:
                            print(
                                "Epoch: {}\tTotal training loss: {}\tTest {}: {}".format(
                                    epoch, loss_val, self.reward_type, error_test_val
                                )
                            )

                        if (
                            np.isnan(loss_val) or loss_val > 1000
        ...
        error_expr_sorted = sorted(
            zip(error_test_final, r2_test_final, eq_list), key=lambda x: x[0]
        )  # List of (error, r2, expr)
    ...
    def predict(self, X):
        variable = self.vars_name
        values = {x: X[:, idx] for idx, x in enumerate(variable)}
        y_pred = sp.lambdify(variable, self.best_expression)(**values)
        return y_pred
...
if __name__ == "__main__":
    ...
    SR = DySymNetRegressor(config=config, func="x_1 + x_2", func_name="x_1+x_2")
    x_numpy = SR.X.detach().numpy()
    eq, R2, error, relative_error = SR.solve_environment()
    print(SR.predict(x_numpy))
    print("Expression: ", eq)
    print("R2: ", R2)
    print("error: ", error)
