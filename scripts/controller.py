...
class Agent(nn.Module):
    def __init__(
        ...
        if self.auto:
            self.n_layer_decoder = nn.Linear(
                hidden_size, len(self.n_layers), device=device
            )
            self.num_funcs_layer_decoder = nn.Linear(
                hidden_size, len(self.num_funcs_layer), device=device
            )
            self.max_input_size = max(len(self.n_layers), len(self.num_funcs_layer))
            self.dynamic_lstm_cell = nn.LSTMCell(
                self.max_input_size, hidden_size, device=device
            )
            self.embedding = nn.Linear(
                self.num_funcs_avail, len(self.num_funcs_layer), device=device
            )

        self.lstm_cell = nn.LSTMCell(input_size, hidden_size, device=device)
        self.decoder = nn.Linear(
            hidden_size, self.num_funcs_avail, device=device
    ...
    def forward(self, input):
        if self.auto:
            if input.shape[-1] < self.max_input_size:
                input = nn.functional.pad(
                    input, (0, self.max_input_size - input.shape[0]), "constant", 0
                )

            assert (
                input.shape[-1] == self.max_input_size
            ), "Error: the input dim of the first step is not equal to the max dim"

            h_t, c_t = self.hidden

            # Sample the number of layers first
            h_t, c_t = self.dynamic_lstm_cell(
                input, (h_t, c_t)
            )  # [batch_size, hidden_size]
            ...
            if input.shape[-1] < self.max_input_size:
                input = nn.functional.pad(
                    input, (0, self.max_input_size - input.shape[-1]), "constant", 0
                )
            h_t, c_t = self.dynamic_lstm_cell(input, (h_t, c_t))
            n_funcs_layer_logits = self.num_funcs_layer_decoder(
                h_t
            )  # [batch_size, len(num_funcs_layer)]
            ...
            action_index3 = dist.sample((num_funcs_layer,)).transpose(
                0, 1
            )  # [num_layers, num_func_layer]
            # print("action_index: ", action_index)
            log_probs = dist.log_prob(action_index3.transpose(0, 1)).transpose(
                0, 1
            )  # [num_layers, num_func_layer] compute the log probability of the sampled action
            entropies = (
                dist.entropy()
            )  # [num_layers] compute the entropy of the action distribution
        ...
        # Fix the number of layers and the number of operators per layer, only sample the operators, each layer is different
            ...
            probs = F.softmax(outputs, dim=-1)
            dist = Categorical(probs=probs)
            action_index = dist.sample((self.num_funcs_layer,)).transpose(
                0, 1
            )  # [num_layers, num_func_layer]
            # print("action_index: ", action_index)
            log_probs = dist.log_prob(action_index.transpose(0, 1)).transpose(
                0, 1
            )  # [num_layers, num_func_layer]
