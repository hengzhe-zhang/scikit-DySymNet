...
class Params:
    ...
    DOMAIN = (
        -1,
        1,
    )  # Domain of dataset - range from which we sample x. Default (-1, 1)
    # DOMAIN = np.array([[0, -1, -1], [1, 1, 1]])   # Use this format if each input variable has a different domain
    N_TEST = 100  # Size of test dataset
    DOMAIN_TEST = (
        -1,
        1,
    )  # Domain of test dataset - should be larger than training domain to test extrapolation. Default (-2, 2)
