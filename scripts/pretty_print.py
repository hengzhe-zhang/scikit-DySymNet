...
def sym_pp(
    W_list, funcs, var_names, threshold=0.01, n_double=None, add_bias=False, biases=None
):
...
def network(weights, funcs, var_names, threshold=0.01, add_bias=False, biases=None):
    ...
    Returns:
        Simplified sympy expression."""
    n_double = [
        functions.count_double(funcs_per_layer) for funcs_per_layer in funcs.values()
    ]
...
def filter_expr(expr, threshold=0.01):
    ...
    for arg in expr.args:
        if (
            arg.is_constant() and abs(arg) > threshold
        ):  # hack way to check if it's a number
