...
class SymbolicLayer(nn.Module):
    ...
    def __init__(
        ...
        self.funcs = [
            func.torch for func in funcs
        ]  # Convert functions to list of PyTorch functions
        self.n_double = functions.count_double(
            funcs
        )  # Number of activation functions that take 2 inputs
        self.n_single = (
            self.n_funcs - self.n_double
        ...
        if self.initial_weight is not None:  # use the given initial weight
            ...
            self.W = nn.Parameter(
                torch.fmod(
                    torch.normal(
                        mean=0.0, std=init_stddev, size=(in_dim, self.out_dim)
                    ),
                    2,
                )
            )
            if add_bias:
                self.b = nn.Parameter(
                    torch.fmod(
                        torch.normal(mean=0.0, std=init_stddev, size=(1, self.out_dim)),
                        2,
                    )
                )
    ...
    def forward(self, x):  # used to be __call__
        ...
        while out_i < self.n_single:
            self.output.append(
                self.funcs[out_i](g[:, in_i])
            )  # g[:, in_i] is the input to the activation function
            in_i += 1
            out_i += 1
...
class SymbolicNet(nn.Module):
    ...
    def get_weights_tensor(self):
        ...
        return [
            self.hidden_layers[i].get_weight_tensor() for i in range(self.depth)
...
class SymbolicLayerL0(SymbolicLayer):
    ...
    def quantile_concrete(self, u):
        ...
        y = torch.sigmoid(
            (torch.log(u) - torch.log(1.0 - u) + self.qz_log_alpha) / self.beta
        )
    ...
    def sample_u(self, shape, reuse_u=False):
        ...
        if self.eps is None or not reuse_u:
            ...
            self.eps = (
                torch.rand(size=shape).to(device) * (1 - 2 * self.epsilon)
                + self.epsilon
            )
        return self.eps
    ...
    def sample_z(self, batch_size, sample=True):
        ...
        if sample:
            ...
            return torch.clamp(
                pi * (self.zeta - self.gamma) + self.gamma, min=0.0, max=1.0
            )
    ...
    def sample_weights(self, reuse_u=False):
        z = self.quantile_concrete(
            self.sample_u((self.in_dim, self.out_dim), reuse_u=reuse_u)
        )
...
class SymbolicNetL0(nn.Module):
    ...
    def __init__(
        ...
        if initial_weights is not None:
            ...
            layers = [
                SymbolicLayerL0(
                    funcs=funcs, init_stddev=init_stddev[i], in_dim=layer_in_dim[i]
                )
                for i in range(self.depth)
            ]
            # Initialize weights for last layer (without activation functions)
            self.output_weight = nn.Parameter(
                torch.rand(size=(self.hidden_layers[-1].n_funcs, 1)) * 2
            )
    ...
    def get_loss(self):
        return torch.sum(
            torch.stack([self.hidden_layers[i].loss() for i in range(self.depth)])
        )
    ...
    def get_weights(self):
        ...
        # First part is iterating over hidden weights. Then append the output weight.
        return [
            self.hidden_layers[i].get_weight().cpu().detach().numpy()
            for i in range(self.depth)
        ] + [self.output_weight.cpu().detach().numpy()]
